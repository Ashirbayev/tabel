$(document).ready(function(){    
    function printit(){
      ...
      unloadMessage();
      //$('.header-footer-checkbox').attr('checked', false);
    }
    ...
    $('.printBtn').click(function(e){
        ...
        var components = {
            ...
            messageBox: function(message){
                return "<div class='printMessageBox' style='\
                position:fixed;\
                top:50%; left:50%;\
                text-align:center;\
        ...
        $("body").append(components.messageBox('Печать'));
        $(".printMessageBox").css("opacity", 0);
        if(!$('#printPage')[0]){
                $("body").append(components.iframe(href));
                $('#printPage').bind("load",function() {  printit();  })
            }else{
                $('#printPage').attr("src", href);
            }        
        return false; 
    });     
});